...
func (s *Service) Start(ctx context.Context, id uuid.UUID) (*Release, error) {
	return s.repo.Transition(ctx, id, func(ctx context.Context, tx pgx.Tx, rel *Release) error {
		switch rel.Phase {
		case PhaseCreated, PhasePaused, PhaseRolledBack:
		default:
			return pkg.ErrConflict("发布当前状态不可 start（phase=" + string(rel.Phase) + ")")
		}
		if err := s.validatePair(ctx, tx, rel); err != nil {
			return err
		}
		cw := rel.CanaryWeight
		if cw == 0 {
		...
		if err := applyWeights(ctx, tx, rel, cw); err != nil {
			return err
		}
		if _, err := updateReleaseState(ctx, tx, rel.ID, PhaseRunning, cw, true, false); err != nil {
			return err
		}
		return insertEvent(ctx, tx, rel.ID, ActionStart, intPtr(0), intPtr(cw),
...
func (s *Service) Pause(ctx context.Context, id uuid.UUID) (*Release, error) {
	return s.repo.Transition(ctx, id, func(ctx context.Context, tx pgx.Tx, rel *Release) error {
		...
		if _, err := updateReleaseState(ctx, tx, rel.ID, PhasePaused, rel.CanaryWeight, false, false); err != nil {
			return err
		}
		return insertEvent(ctx, tx, rel.ID, ActionPause, nil, nil,
...
func (s *Service) Resume(ctx context.Context, id uuid.UUID) (*Release, error) {
	return s.repo.Transition(ctx, id, func(ctx context.Context, tx pgx.Tx, rel *Release) error {
		...
		if _, err := updateReleaseState(ctx, tx, rel.ID, PhaseRunning, rel.CanaryWeight, false, false); err != nil {
			return err
		}
		return insertEvent(ctx, tx, rel.ID, ActionResume, nil, nil,
...
func (s *Service) SetWeight(ctx context.Context, id uuid.UUID, w int) (*Release, error) {
	...
	return s.repo.Transition(ctx, id, func(ctx context.Context, tx pgx.Tx, rel *Release) error {
		...
		if err := applyWeights(ctx, tx, rel, w); err != nil {
			return err
		}
		if _, err := updateReleaseState(ctx, tx, rel.ID, rel.Phase, w, false, false); err != nil {
			return err
		}
		return insertEvent(ctx, tx, rel.ID, ActionWeight, intPtr(from), intPtr(w),
...
func (s *Service) Promote(ctx context.Context, id uuid.UUID) (*Release, error) {
	return s.repo.Transition(ctx, id, func(ctx context.Context, tx pgx.Tx, rel *Release) error {
		...
		if err := setVersionStatus(ctx, tx, rel.CanaryVersionID, "stable"); err != nil {
			return err
		}
		if err := setVersionWeight(ctx, tx, rel.CanaryVersionID, 100); err != nil {
			return err
		}
		if err := setVersionStatus(ctx, tx, rel.StableVersionID, "draining"); err != nil {
			return err
		}
		if err := setVersionWeight(ctx, tx, rel.StableVersionID, 0); err != nil {
			return err
		}
		if _, err := updateReleaseState(ctx, tx, rel.ID, PhaseCompleted, 100, false, true); err != nil {
			return err
		}
		return insertEvent(ctx, tx, rel.ID, ActionPromote, intPtr(rel.CanaryWeight), intPtr(100),
...
func (s *Service) Rollback(ctx context.Context, id uuid.UUID) (*Release, error) {
	return s.repo.Transition(ctx, id, func(ctx context.Context, tx pgx.Tx, rel *Release) error {
		...
		if err := setVersionWeight(ctx, tx, rel.CanaryVersionID, 0); err != nil {
			return err
		}
		if err := setVersionStatus(ctx, tx, rel.CanaryVersionID, "standby"); err != nil {
			return err
		}
		if err := setVersionStatus(ctx, tx, rel.StableVersionID, "stable"); err != nil {
			return err
		}
		if err := setVersionWeight(ctx, tx, rel.StableVersionID, 100); err != nil {
			return err
		}
		if _, err := updateReleaseState(ctx, tx, rel.ID, PhaseRolledBack, 0, false, true); err != nil {
			return err
		}
		return insertEvent(ctx, tx, rel.ID, ActionRollback, intPtr(rel.CanaryWeight), intPtr(0),
			"rollback: canary drained to 0%, stable weight restored to 100%")
	})
}

// validatePair 校验 stable/canary 版本属于同一部署，且无其它活跃(same service)发布。
func (s *Service) validatePair(ctx context.Context, tx pgx.Tx, rel *Release) error {
	sw, _, _, sDep, err := versionRef(ctx, tx, rel.StableVersionID)
	if err != nil {
		return err
	}
	_, _, _, cDep, err := versionRef(ctx, tx, rel.CanaryVersionID)
	if err != nil {
		return err
	}
	if sDep != cDep {
...
func applyWeights(ctx context.Context, tx pgx.Tx, rel *Release, w int) error {
	if err := setVersionWeight(ctx, tx, rel.CanaryVersionID, w); err != nil {
		return err
	}
	return setVersionWeight(ctx, tx, rel.StableVersionID, 100-w)
}
