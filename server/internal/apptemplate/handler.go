...
// Handler 暴露 Template 的 Management API。
type Handler struct {
	repo *Repository
}
...
// List GET /api/admin/templates?status=&limit=&offset=
...
func (h *Handler) Get(c fiber.Ctx) error {
	...
	if err != nil {
		return pkg.Err(c, pkg.ErrValidation("无效的模板 ID"))
	}
	t, err := h.repo.Get(c.Context(), id)
	if err != nil {
		return pkg.Err(c, err)
	}
	return pkg.OK(c, t)
...
// Enable POST /api/admin/templates/:id/enable
func (h *Handler) Enable(c fiber.Ctx) error { return h.setStatus(c, StatusActive) }

// Disable POST /api/admin/templates/:id/disable
