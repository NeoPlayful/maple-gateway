...
func (cq *CertificateQuery) QueryDomain() *DomainQuery {
...
func (cq *CertificateQuery) Clone() *CertificateQuery {
	...
	return &CertificateQuery{
		...
		withDomain: cq.withDomain.Clone(),
...
func (cq *CertificateQuery) WithDomain(opts ...func(*DomainQuery)) *CertificateQuery {
	query := (&DomainClient{config: cq.config}).Query()
...
//		GroupBy(certificate.FieldDomainID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Certificate.Query().
//		Select(certificate.FieldDomainID).
//		Scan(ctx, &v)
...
func (cq *CertificateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Certificate, error) {
	...
	if query := cq.withDomain; query != nil {
		if err := cq.loadDomain(ctx, query, nodes, nil,
			func(n *Certificate, e *Domain) { n.Edges.Domain = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CertificateQuery) loadDomain(ctx context.Context, query *DomainQuery, nodes []*Certificate, init func(*Certificate), assign func(*Certificate, *Domain)) error {
...
func (cq *CertificateQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withDomain != nil {
			_spec.Node.AddColumnOnce(certificate.FieldDomainID)
		}
	}
