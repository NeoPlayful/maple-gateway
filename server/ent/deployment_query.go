...
type DeploymentQuery struct {
	...
	withVersions *DeploymentVersionQuery
...
func (dq *DeploymentQuery) QueryVersions() *DeploymentVersionQuery {
	query := (&DeploymentVersionClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DeploymentQuery) Clone() *DeploymentQuery {
	...
	return &DeploymentQuery{
		...
		withVersions: dq.withVersions.Clone(),
...
func (dq *DeploymentQuery) WithVersions(opts ...func(*DeploymentVersionQuery)) *DeploymentQuery {
	query := (&DeploymentVersionClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withVersions = query
	return dq
}
...
//		GroupBy(deployment.FieldServiceID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Deployment.Query().
//		Select(deployment.FieldServiceID).
//		Scan(ctx, &v)
...
func (dq *DeploymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Deployment, error) {
	...
	if query := dq.withVersions; query != nil {
		if err := dq.loadVersions(ctx, query, nodes,
			func(n *Deployment) { n.Edges.Versions = []*DeploymentVersion{} },
			func(n *Deployment, e *DeploymentVersion) { n.Edges.Versions = append(n.Edges.Versions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DeploymentQuery) loadVersions(ctx context.Context, query *DeploymentVersionQuery, nodes []*Deployment, init func(*Deployment), assign func(*Deployment, *DeploymentVersion)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deploymentversion.FieldDeploymentID)
	}
	query.Where(predicate.DeploymentVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(deployment.VersionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
