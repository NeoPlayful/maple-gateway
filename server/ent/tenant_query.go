...
func (tq *TenantQuery) QueryDomains() *DomainQuery {
	query := (&DomainClient{config: tq.config}).Query()
...
func (tq *TenantQuery) QueryServices() *ServiceQuery {
...
func (tq *TenantQuery) WithDomains(opts ...func(*DomainQuery)) *TenantQuery {
	query := (&DomainClient{config: tq.config}).Query()
...
func (tq *TenantQuery) WithServices(opts ...func(*ServiceQuery)) *TenantQuery {
	query := (&ServiceClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TenantQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tenant, error) {
	...
	if query := tq.withServices; query != nil {
		if err := tq.loadServices(ctx, query, nodes,
			func(n *Tenant) { n.Edges.Services = []*Service{} },
			func(n *Tenant, e *Service) { n.Edges.Services = append(n.Edges.Services, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TenantQuery) loadDomains(ctx context.Context, query *DomainQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Domain)) error {
...
func (tq *TenantQuery) loadServices(ctx context.Context, query *ServiceQuery, nodes []*Tenant, init func(*Tenant), assign func(*Tenant, *Service)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Tenant)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(service.FieldTenantID)
	}
	query.Where(predicate.Service(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tenant.ServicesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
